...
const RequestFriend = () => {
  ...
  const loadInitial = async () => {
    ...
    try {
      ...
      const friendData = res.friendList.map((f: any) => ({
        ...f.acceptMember,
        friendId: f.friendId,
      }));
  ...
  const loadMore = async () => {
    ...
    try {
      ...
      // console.error("친구 요청 추가 로딩 실패:", error);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleDeleteFriend = (friendId: number) => {
    setFriendList((prev) => prev.filter((f) => f.friendId !== friendId));
  };
  ...
  return (
    <div className="flex flex-col gap-4 mb-5">
      ...
      {!isLoading && friendList.length === 0 && (
        <div className="text-sm text-center text-gray-400">보낸 친구 요청이 없어요</div>
      )}
    </div>
  );
};
